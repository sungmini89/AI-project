...
const ComprehensiveDebugPanel: React.FC = () => {
  ...
  const testEmotionAnalysis = async () => {
    ...
    await runTest(
      async () => {
        ...
        if (
          ...
          result.primaryEmotion !== "excited"
        ) {
          throw new Error(
            `Expected positive emotion, got ${result.primaryEmotion}`
          );
        }
    ...
    await runTest(
      async () => {
        ...
        const mixedResult = await emotionAnalysisService.analyzeEmotion(
          "I am happy but also sad"
        );
        if (
          !mixedResult.primaryEmotion ||
          typeof mixedResult.confidence !== "number"
        ) {
  ...
  const runAllTests = async () => {
    ...
    setIsRunning(true);
    setTestSuites((prev) =>
      prev.map((suite) => ({ ...suite, tests: [], status: "pending" }))
    );

    try {
      // Mark suites as running
      setTestSuites((prev) =>
        prev.map((suite) => ({ ...suite, status: "running" }))
      );
      ...
      setTestSuites((prev) =>
        prev.map((suite) =>
          suite.status === "running" ? { ...suite, status: "completed" } : suite
        )
      );
    }
  };
  ...
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-4">
                ...
                suite.tests.map((test, testIndex) => (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2 text-xs text-gray-500">
                      ...
                      {test.error && (
                        <span
                          className="text-red-600 max-w-48 truncate"
                          title={test.error}
                        >
                          {test.error}
                        </span>
      ...
      <div className="text-xs text-gray-500 bg-gray-100 p-2 rounded mt-4">
        ⚠️ 개발 모드에서만 표시됩니다. 이 패널은 애플리케이션의 모든 핵심 기능을
        테스트합니다.
      </div>
    </div>
  );
