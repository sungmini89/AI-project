...
interface ColorPaletteProps {
  ...
  onExport?: (palette: ColorPaletteType, format: 'png' | 'svg' | 'json' | 'css') => void;
        ...
        <div className="flex items-start justify-between">
          <div className="flex flex-col gap-1">
            <CardTitle className="flex items-center gap-2">
              {palette.name || `${harmonyTypeLabels[palette.harmonyType]} 팔레트`}
              {palette.source && (
            ...
            <CardDescription className="flex items-center gap-4">
              ...
              <span className="flex items-center gap-1">
                ...
                <span className={cn("font-medium", accessibilityInfo.color)}>
                  {accessibilityInfo.label} ({Math.round(palette.accessibilityScore * 100)}%)
                </span>
              </span>
              {palette.keyword && (
