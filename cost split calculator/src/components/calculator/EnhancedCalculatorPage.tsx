 ...
 * 향상된 계산기 페이지 컴포넌트
 *
 * 영수증 OCR과 수동 입력을 지원하는 스마트 분할 계산기의 메인 페이지입니다.
 * 3단계 프로세스(항목 입력 → 참가자 할당 → 결과 확인)로 구성되어 있으며,
 * OCR을 통한 자동 영수증 분석과 수동 입력을 모두 지원합니다.
 *
 * @description
 * **주요 기능:**
 * - 영수증 촬영/업로드를 통한 자동 항목 추출
 * - 수동으로 항목과 참가자 정보 입력
 * - 참가자별 항목 할당 및 분할 계산
 * - 계산 결과 저장 및 히스토리 관리
 *
 * **프로세스 단계:**
 * 1. **항목 입력**: 영수증 OCR 또는 수동 입력
 * 2. **참가자 할당**: 참가자 추가 및 항목별 할당
 * 3. **결과 확인**: 분할 계산 결과 및 저장
 *
 * @param props - 컴포넌트 속성
 * @param props.className - 추가 CSS 클래스명
 *
 * @returns {JSX.Element} 향상된 계산기 페이지 UI
 */
const EnhancedCalculatorPage: React.FC<EnhancedCalculatorPageProps> = ({
  ...
  const [calculationStatus, setCalculationStatus] = useState<
    "pending" | "completed" | "shared"
  >("pending");

  // OCR 관련 상태
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [analysisProgress, setAnalysisProgress] =
    useState<AnalysisProgressType | null>(null);
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    try {
      const result = await receiptAnalyzer.analyzeReceipt(file, (progress) => {
  ...
  const handleUseAnalysisResults = useCallback(
  ...
  const updateBillItem = useCallback(
    (id: string, updates: Partial<BillItem>) => {
      setBillItems((prev) =>
  ...
  const deleteParticipant = useCallback((id: string) => {
    ...
    setBillItems((prev) =>
      prev.map((item) => ({
        ...item,
        assignedParticipants:
          item.assignedParticipants?.filter((pid) => pid !== id) || [],
      }))
    );
  }, []);
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <TabsContent value="receipt" className="space-y-6 mt-6">
          ...
          {analysisProgress && <AnalysisProgress progress={analysisProgress} />}
                ...
                <div className="flex flex-wrap gap-2">
                  {participants.map((participant) => {
                    ...
                    return (
                      ...
                      >
                        {participant.name ||
                          `참가자 ${participants.indexOf(participant) + 1}`}
                      </button>
                    );
                  })}
                </div>
            ...
            onClick={() => {
              ...
              const savedItem = historyService.add({
                ...
                details: {
                  ...
                  participantTotals: participantTotals.reduce(
                    (acc, { participant, total }) => {
        ...
        <CardHeader className="hud-element rounded-t-lg">
          ...
          <div className="flex items-center space-x-4 mt-4">
            {[
              ...
              const isCompleted =
                ["input", "participants", "results"].indexOf(currentStep) >
