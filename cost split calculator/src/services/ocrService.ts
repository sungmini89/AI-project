...
/**
 * 영수증 OCR(광학 문자 인식) 처리 서비스
 *
 * Tesseract.js를 사용하여 영수증 이미지에서 텍스트를 추출하고,
 * 한국어 영수증에 특화된 파싱 로직으로 상품명과 가격을 분석합니다.
 *
 * @description
 * **주요 기능:**
 * - 다중 스케일 이미지 처리로 인식률 향상
 * - 한국어 영수증 키워드 기반 총액 추출
 * - 상품명과 가격 자동 파싱
 * - 이미지 대비 강화로 OCR 정확도 개선
 *
 * **처리 과정:**
 * 1. 이미지를 여러 스케일(1.2x ~ 2.5x)로 변환
 * 2. Tesseract.js로 한국어+영어 텍스트 추출
 * 3. 총액 키워드 검색 및 금액 추출
 * 4. 상품명 패턴 분석 및 항목 목록 생성
 *
 * @class OCRService
 */
export class OCRService {
      ...
      for (const scale of scales) {
        ...
        try {
          ...
          if (result.totalAmount > 0) {
            console.log(
              `✅ 스케일 ${scale}x에서 성공: 총액 ${result.totalAmount}원`
            );
            bestResult = result;
   ...
   * 특정 스케일로 이미지 처리
   */
  private async processImageAtScale(
    imageFile: File,
    scale: number
  ): Promise<OCRResult> {
  ...
  private extractTotalAmount(text: string): number {
      ...
      if (matches) {
        for (const match of matches) {
          ...
          if (amount >= 1000) {
            ...
            console.log(
              `💰 콤마 패턴 발견: ${amount}원 (${match}) → 총 ${currentCount + 1}회`
            );
          }
        }
      }
    ...
    if (amountCounts.size > 0) {
      ...
      console.log(
        `✅ 빈도 기반 총액 결정: ${totalAmount}원 (빈도 ${maxCount}회 + 최대값)`
      );
      return totalAmount;
    }
  ...
  private extractItems(text: string): Array<{ name: string; price: number }> {
    ...
    for (const line of lines) {
      ...
      if (totalAmount > 0) {
        ...
        if (diffRatio > 0.5) {
          console.log(
            `🔄 금액 보정: ${price}원 → ${totalAmount}원 (차이 ${(diffRatio * 100).toFixed(1)}%)`
          );
          price = totalAmount;
        }
      }
