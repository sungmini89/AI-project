 ...
 * 계산 히스토리 관리 서비스 클래스
 *
 * 사용자의 분할 계산 기록을 localStorage에 저장하고 관리합니다.
 * 계산 결과의 저장, 조회, 수정, 삭제 기능을 제공하며,
 * 검색과 통계 데이터 조회 기능도 포함합니다.
 *
 * @description
 * **주요 기능:**
 * - 계산 결과 영구 저장 (localStorage)
 * - 히스토리 목록 조회 및 검색
 * - 계산 결과 수정 및 삭제
 * - 통계 데이터 제공 (최근 12개월)
 * - 초기 샘플 데이터 제공
 *
 * **데이터 구조:**
 * - 각 히스토리 항목은 고유 ID와 메타데이터 포함
 * - 참가자 목록, 상품 목록, 분할 결과 등 상세 정보 저장
 * - 상태 관리 (완료, 진행중, 공유됨)
 *
...
class HistoryService {
  ...
  getStatsData(): HistoryItem[] {
    ...
    return items.filter((item) => new Date(item.date) >= twelveMonthsAgo);
  }
}
