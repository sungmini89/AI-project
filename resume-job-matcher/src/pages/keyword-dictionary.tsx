...
export default function KeywordDictionary() {
  ...
  const keywordDatabase = [
    ...
    { 
      ...
      relatedSkills: ['JSX', 'Virtual DOM', 'Component', 'Hooks', 'State Management'],
      industryDemand: 95,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Template Syntax', 'Composition API', 'Options API', 'Vuex'],
      industryDemand: 82,
      salaryImpact: 'high'
    },
    { 
    ...
    { 
      ...
      relatedSkills: ['DOM', 'Event Handling', 'Async/Await', 'Promise', 'Closure'],
      industryDemand: 98,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Flexbox', 'Grid', 'Responsive Design', 'Animation', 'SASS'],
      industryDemand: 92,
      salaryImpact: 'medium'
    },
    { 
      ...
      relatedSkills: ['Semantic HTML', 'Web Accessibility', 'SEO', 'Web Standards'],
      industryDemand: 94,
      salaryImpact: 'medium'
    },
    ...
    { 
      ...
      relatedSkills: ['npm', 'Express', 'REST API', 'WebSocket', 'Microservices'],
      industryDemand: 87,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Django', 'Flask', 'Data Science', 'Machine Learning', 'API'],
      industryDemand: 91,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Spring Framework', 'Hibernate', 'Maven', 'Gradle', 'JUnit'],
      industryDemand: 89,
      salaryImpact: 'high'
    },
    { 
    ...
    { 
      ...
      relatedSkills: ['SQL', 'Database Design', 'Query Optimization', 'Indexing'],
      industryDemand: 85,
      salaryImpact: 'medium'
    },
    { 
      ...
      relatedSkills: ['Advanced SQL', 'JSONB', 'Full-text Search', 'Extensions'],
      industryDemand: 82,
      salaryImpact: 'high'
    },
    { 
    ...
    { 
      ...
      relatedSkills: ['Containerization', 'Kubernetes', 'Docker Compose', 'Microservices'],
      industryDemand: 86,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Container Orchestration', 'Helm', 'Service Mesh', 'CI/CD'],
      industryDemand: 81,
      salaryImpact: 'high'
    },
    { 
      ...
      relatedSkills: ['Branch Management', 'Merge Conflicts', 'Pull Request', 'CI/CD'],
      industryDemand: 97,
      salaryImpact: 'medium'
    },
    ...
    { 
      ...
      relatedSkills: ['Dart', 'Widget', 'State Management', 'Platform Channels'],
      industryDemand: 74,
      salaryImpact: 'high'
    },
    ...
    { 
      ...
      relatedSkills: ['Python', 'TensorFlow', 'PyTorch', 'Scikit-learn', 'Data Analysis'],
      industryDemand: 88,
      salaryImpact: 'high'
    },
    { 
  ...
  const categories = [
    ...
    { id: 'frontend', name: '프론트엔드', icon: Code, count: keywordDatabase.filter(k => k.category === 'frontend').length },
    { id: 'backend', name: '백엔드', icon: Server, count: keywordDatabase.filter(k => k.category === 'backend').length },
    { id: 'database', name: '데이터베이스', icon: Database, count: keywordDatabase.filter(k => k.category === 'database').length },
    { id: 'cloud', name: '클라우드', icon: Cloud, count: keywordDatabase.filter(k => k.category === 'cloud').length },
    { id: 'devops', name: 'DevOps', icon: Shield, count: keywordDatabase.filter(k => k.category === 'devops').length },
    { id: 'mobile', name: '모바일', icon: Smartphone, count: keywordDatabase.filter(k => k.category === 'mobile').length },
    { id: 'data', name: '데이터/AI', icon: BarChart3, count: keywordDatabase.filter(k => k.category === 'data').length }
  ];
  ...
  const filteredKeywords = useMemo(() => {
    return keywordDatabase.filter(keyword => {
      const matchesSearch = keyword.keyword.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           ...
                           keyword.synonyms.some(syn => syn.toLowerCase().includes(searchTerm.toLowerCase())) ||
                           keyword.relatedSkills.some(skill => skill.toLowerCase().includes(searchTerm.toLowerCase()));
      
      const matchesCategory = selectedCategory === 'all' || keyword.category === selectedCategory;
      const matchesLevel = selectedLevel === 'all' || keyword.level === selectedLevel;
  ...
  const getWeightColor = (weight: number) => {
    ...
    if (weight >= 8) return 'text-orange-600 bg-orange-100 dark:bg-orange-900/20';
    if (weight >= 7) return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/20';
    return 'text-green-600 bg-green-100 dark:bg-green-900/20';
  };

  const getSalaryImpactColor = (impact: string) => {
    if (impact === 'high') return 'text-green-600 bg-green-100 dark:bg-green-900/20';
    if (impact === 'medium') return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/20';
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            <div className="text-center p-4 rounded-lg bg-white/50 dark:bg-slate-800/50">
              <div className="text-2xl font-bold text-primary">{keywordDatabase.length}</div>
              <div className="text-sm text-muted-foreground">총 키워드</div>
            </div>
            <div className="text-center p-4 rounded-lg bg-white/50 dark:bg-slate-800/50">
              <div className="text-2xl font-bold text-green-600">{keywordDatabase.filter(k => k.level === 'high').length}</div>
            ...
            <div className="text-center p-4 rounded-lg bg-white/50 dark:bg-slate-800/50">
              <div className="text-2xl font-bold text-purple-600">{filteredKeywords.length}</div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-xs">
                          {categories.find(c => c.id === keyword.category)?.name}
                        </Badge>
                        <Badge className={getSalaryImpactColor(keyword.salaryImpact)}>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold">
                    {searchTerm ? `"${searchTerm}" 검색 결과` : categories.find(c => c.id === selectedCategory)?.name || '전체'} 키워드
                        ...
                        <CardHeader className="pb-4">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="text-right">
                              <div className="text-sm text-muted-foreground">업계 수요</div>
                              <div className="text-2xl font-bold text-primary">{keyword.industryDemand}%</div>
                            </div>
                          </div>
                        </CardHeader>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          {keyword.relatedSkills.length > 0 && (
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1">
                                {keyword.relatedSkills.slice(0, 5).map((skill, i) => (
            ...
            {[
              {
                ...
                description: '높은 가중치를 가진 키워드를 이력서에 포함시켜 ATS 통과율을 높이세요.',
                color: 'from-yellow-500 to-orange-600'
              },
              {
                icon: Target,
                title: '동의어 활용하기',
                description: '같은 기술을 다양한 표현으로 기술하여 키워드 매칭률을 향상시키세요.',
                color: 'from-green-500 to-teal-600'
              },
              {
                icon: TrendingUp,
                title: '연관 기술 함께 언급',
                description: '메인 키워드와 함께 연관 기술들을 언급하여 전문성을 어필하세요.',
