/**
 * 통합 색상 서비스
 * 다중 API 통합 및 오프라인 폴백 지원
 */

import { ColorTheory, KeywordMapper, AccessibilityChecker } from '../algorithms';
import type { HSLColor, HarmonyType, ColorPalette, ColorSwatch, GeneratePaletteResult } from '../types/color';
...
export class ColorService {
   ...
   * 메인 팔레트 생성 메서드
   */
  async generatePalette(options: GeneratePaletteOptions): Promise<GeneratePaletteResult> {
    ...
    try {
      ...
      // 기본 색상 결정
      const baseColor = options.baseColor || this.keywordMapper.mapKeywordToColor(options.keyword || 'blue');
      ...
      switch (this.config.mode) {
        case 'free':
          try {
            if (this.config.primaryAPI === 'colormind') {
              colors = await this.generateWithColormind(baseColor, options.harmonyType);
              source = 'colormind';
            } else if (this.config.primaryAPI === 'thecolorapi') {
              colors = await this.generateWithTheColorAPI(baseColor, options.harmonyType);
              source = 'thecolorapi';
            } else {
        ...
        case 'custom':
          if (this.config.enableHuggingFace && this.config.apiKey) {
            try {
              colors = await this.generateWithHuggingFace(options.keyword || 'blue', baseColor);
              source = 'huggingface';
            } catch (error) {
              colors = this.generateWithLocal(baseColor, options.harmonyType);
              source = 'local';
      ...
      // 접근성 점수 계산
      const accessibilityReport = this.accessibilityChecker.checkPaletteAccessibility(colors);
      
      // 결과 팔레트 생성
      const colorSwatches = this.hslArrayToColorSwatches(colors);
      const palette: ColorPalette = {
        id: `palette-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        name: `${options.keyword || 'Generated'} - ${this.getHarmonyName(options.harmonyType)}`,
      ...
      const palette: ColorPalette = {
        id: `palette-fallback-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        name: `${options.keyword || 'Generated'} - ${this.getHarmonyName(options.harmonyType)} (폴백)`,
        colors: colorSwatches,
        harmonyType: options.harmonyType,
   ...
   */
  private async generateWithColormind(baseColor: HSLColor, harmonyType: HarmonyType): Promise<HSLColor[]> {
    ...
    const colors = data.result.map((rgbArray: number[]) => 
      this.colorTheory.rgbToHsl({ r: rgbArray[0], g: rgbArray[1], b: rgbArray[2] })
    );
   ...
   */
  private async generateWithTheColorAPI(baseColor: HSLColor, harmonyType: HarmonyType): Promise<HSLColor[]> {
    const hexColor = this.colorTheory.hslToHex(baseColor).replace('#', '');
    
    const response = await fetch(
      `https://www.thecolorapi.com/scheme?hex=${hexColor}&mode=${this.getTheColorAPIMode(harmonyType)}&count=5`,
      { signal: AbortSignal.timeout(this.config.timeout) }
    );
   ...
   */
  private generateWithLocal(baseColor: HSLColor, harmonyType: HarmonyType): HSLColor[] {
   ...
   */
  private async generateWithHuggingFace(keyword: string, baseColor: HSLColor): Promise<HSLColor[]> {
    if (!this.config.apiKey) {
      throw new Error('Hugging Face API 키가 필요합니다');
    }

    // 실제 구현시에는 적절한 Hugging Face 색상 모델 사용
    const response = await fetch('https://api-inference.huggingface.co/models/color-palette', {
      method: 'POST',
      headers: {
   ...
   * 색상 개수 조정
   */
  private adjustColorCount(colors: HSLColor[], targetCount: number): HSLColor[] {
    ...
    if (colors.length > targetCount) {
      ...
      while (result.length < targetCount) {
        ...
        const variation = {
          ...baseColor,
          l: Math.max(10, Math.min(90, baseColor.l + (Math.random() - 0.5) * 40))
        };
   ...
   * 조화 규칙을 기존 색상에 적용
   */
  private applyHarmonyToColors(colors: HSLColor[], baseColor: HSLColor, harmonyType: HarmonyType): HSLColor[] {
    // API에서 받은 색상들을 조화 규칙에 맞게 재정렬하고 조정
    const harmonyColors = this.colorTheory.generateHarmony(baseColor, harmonyType);
  ...
  private getTheColorAPIMode(harmonyType: HarmonyType): string {
    switch (harmonyType) {
      case 'complementary': return 'complement';
      case 'analogous': return 'analogic';
      case 'triadic': return 'triad';
      case 'tetradic': return 'quad';
      case 'monochromatic': return 'monochrome';
  ...
  async checkServiceStatus(): Promise<{
    ...
    // TheColorAPI 상태 확인
    try {
      const response = await fetch('https://www.thecolorapi.com/id?hex=ff0000', {
        signal: AbortSignal.timeout(5000)
      });
