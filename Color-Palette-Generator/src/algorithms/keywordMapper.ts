/**
 * @fileoverview 한국어 키워드 기반 색상 매핑 시스템
 *
 * 한국어 키워드를 색상으로 변환하는 지능형 매핑 시스템입니다.
 * 감정, 자연, 계절, 추상적 개념 등 다양한 카테고리의 키워드를
 * HSL 색상 공간의 적절한 색상으로 매핑합니다.
 *
 ...
 *
 * **지원 카테고리:**
 * - 감정 (emotion): 평온함, 열정, 행복, 신뢰, 사랑, 우울
 * - 자연 (nature): 숲, 바다, 하늘, 노을, 꽃, 잔디
 * - 계절 (season): 봄, 여름, 가을, 겨울
 * - 추상 (abstract): 에너지, 미니멀, 럭셔리, 모던, 빈티지
 * - 사물 (object): 태양, 달, 불, 얼음
 * - 개념 (concept): 성공, 창의, 집중, 자유
 *
 * **사용 예시:**
 * ```typescript
 * const mapper = new KeywordMapper();
 * const color = mapper.mapKeywordToColor('바다');
 * const suggestions = mapper.searchKeywords('자연');
 * ```
 */
 ...
 * @property {HSLColor} color - 매핑된 색상
 * @property {KeywordCategory} category - 키워드 카테고리
 * @property {string} description - 색상 설명
 * @property {string[]} synonyms - 동의어 목록
 */
...
export class KeywordMapper {
  ...
  searchKeywords(query: string, limit: number = 5): KeywordMapping[] {
    ...
    for (const mapping of Object.values(this.keywordColorMap)) {
      if (
        mapping.keyword.toLowerCase().includes(normalizedQuery) &&
  ...
  getKeywordInfo(keyword: string): KeywordMapping | null {
    const normalizedKeyword = keyword.trim().toLowerCase();
    return (
      this.keywordColorMap[normalizedKeyword] ||
