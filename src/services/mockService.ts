...
export class MockAIService {
  ...
  private generateMockAnalysis(
    ...
    if (language === 'javascript' || language === 'typescript') {
      if (code.includes('console.log')) {
        issues.push({
          ...
          explanation: '프로덕션 환경에서는 console.log를 제거하는 것이 좋습니다. 적절한 로깅 라이브러리를 사용하거나 디버그 목적의 코드는 제거해주세요.',
      ...
      if (code.includes('var ')) {
        issues.push({
          ...
          message: 'var 키워드 사용',
          explanation: 'ES6+에서는 블록 스코프를 가지는 let과 const 사용을 권장합니다.',
      ...
      if (code.includes('==') && !code.includes('===')) {
        issues.push({
          ...
          explanation: '타입 강제 변환을 피하기 위해 엄격한 동등 비교 연산자(===)를 사용하세요.',
          fix: '== 대신 ===를 사용하세요.'
        });
      }
      ...
      if (code.includes('eval(')) {
        issues.push({
          ...
          explanation: 'eval() 함수는 보안 위험을 초래할 수 있습니다. 코드 인젝션 공격에 취약합니다.',
      ...
      if (lines.length > 50) {
        suggestions.push({
          ...
          example: '큰 함수를 더 작고 재사용 가능한 함수들로 분할하는 것을 고려해보세요.'
        });
      }
      ...
      if (code.includes('function') && code.split('function').length > 5) {
        suggestions.push({
          ...
          example: 'const myFunc = (param) => { ... } 형태로 작성할 수 있습니다.'
        });
      }
    }
    ...
    if (language === 'python') {
      if (code.includes('print(')) {
        issues.push({
          ...
          explanation: '프로덕션 코드에서는 적절한 로깅을 사용하는 것이 좋습니다.',
      ...
      if (code.includes('except:')) {
        issues.push({
          ...
          message: '광범위한 예외 처리',
          explanation: '모든 예외를 포착하는 것보다 특정 예외를 처리하는 것이 좋습니다.',
    ...
    const issueDeduction = issues.reduce((total, issue) => {
      return total + (issue.severity === 'critical' ? 15 : 
    ...
    return {
      ...
      summary: this.generateSummary(issues.length, suggestions.length, language),
      issues,
      suggestions,
      score,
      confidence: 0.85
  ...
  generateScenario(scenario: 'perfect' | 'issues' | 'critical'): Partial<AIAnalysisResult> {
    switch (scenario) {
      case 'perfect':
        return {
          ...
          suggestions: [{
            type: 'optimize',
            priority: 'low',
            description: '코드가 매우 깔끔합니다. 성능 최적화를 고려해볼 수 있습니다.',
          ...
          issues: [
            {
              ...
              explanation: '변수가 null일 수 있는 상황에서 안전하지 않은 접근이 발견되었습니다.',
          ...
          issues: [
            {
              ...
              line: 8,
              message: 'SQL 인젝션 취약점',
              explanation: '사용자 입력이 SQL 쿼리에 직접 삽입되어 SQL 인젝션 공격에 취약합니다.',
