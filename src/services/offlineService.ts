...
export class OfflineAnalysisService {
  ...
  async analyzeWithESLint(
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      // no-implied-eval 검사
      if (line.match(/setTimeout\s*\(\s*['"`]/) || line.match(/setInterval\s*\(\s*['"`]/)) {
        results.push({
          line: lineNumber,
          column: 1,
          severity: 'error',
  ...
  calculateComplexity(
    ...
    return {
      ...
      overall: this.getComplexityLevel(Math.max(totalCyclomatic, totalCognitive))
    };
  }
  ...
  private checkUnusedVars(line: string, lines: string[], index: number): boolean {
  ...
  private extractFunctions(code: string, language: SupportedLanguage): any[] {
    ...
    switch (language) {
      case 'javascript':
      case 'typescript':
        functionRegex = /(?:function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)|([a-zA-Z_$][a-zA-Z0-9_$]*)\s*[:=]\s*(?:function|\([^)]*\)\s*=>))/g;
        break;
      case 'python':
        functionRegex = /def\s+([a-zA-Z_][a-zA-Z0-9_]*)/g;
        break;
      case 'java':
        functionRegex = /(?:public|private|protected)?\s*(?:static\s+)?[a-zA-Z_<>[\]]+\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/g;
        break;
      default:
        return [];
    }
  ...
  private calculateFunctionComplexity(func: any, language: SupportedLanguage): FunctionComplexity {
  ...
  private getComplexityLevel(complexity: number): 'low' | 'medium' | 'high' | 'very-high' {
    if (complexity <= config.analysis.complexity.cyclomaticThreshold) return 'low';
    if (complexity <= config.analysis.complexity.cyclomaticThreshold * 1.5) return 'medium';
    if (complexity <= config.analysis.complexity.cyclomaticThreshold * 2) return 'high';
    return 'very-high';
  }
      ...
      case 'typescript':
        ...
        if (line.includes('.innerHTML')) {
          issues.push({
            ...
            pattern: '.innerHTML',
            suggestion: 'textContent를 사용하거나 적절한 새니타이제이션을 적용하세요'
          });
        }
        break;
