...
import { useCodeStore, useAnalysisStore, useSettingsStore, useUIStore } from '../stores';
...
export const AnalyzePage: React.FC = () => {
  ...
  useEffect(() => {
    const serviceConfig: AIServiceConfig = {
      ...
      provider: apiKeys.gemini ? 'gemini' : apiKeys.cohere ? 'cohere' : undefined
    };

    const newOrchestrator = new AnalysisOrchestrator(serviceConfig);
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      failAnalysis(error instanceof Error ? error.message : '알 수 없는 오류');
  ...
  const handleRealtimeAnalysis = useCallback(async () => {
    ...
    try {
      const results = await orchestrator.analyzeCodeRealtime(currentCode, currentLanguage);
  ...
  }, [orchestrator, preferences.realTimeAnalysis, currentCode, currentLanguage]);

  // 디바운스된 실시간 분석
  useEffect(() => {
    if (!preferences.realTimeAnalysis) return;

    const debounceTimer = setTimeout(handleRealtimeAnalysis, 1000);
    return () => clearTimeout(debounceTimer);
  }, [currentCode, handleRealtimeAnalysis, preferences.realTimeAnalysis]);
  ...
  const handleFormatCode = useCallback(async () => {
    ...
    try {
      ...
      const result = await orchestrator.formatCode(currentCode, currentLanguage);
  ...
  }, [orchestrator, currentCode, currentLanguage, setCode, addNotification, setLoading]);
            ...
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1">
                <CodeEditor
                  ...
                  theme={useSettingsStore.getState().theme === 'dark' ? 'dark' : 'light'}
                  height={600}
                  showMinimap={true}
                />
              </div>
